...
for i in range(len((a))):
    if sum(a[i:i+7])/7 == minn:
        ...
        date_min=j%len(number[0])
    if len(number[mesyac_min])==30:
        date_min+=1

print(f"Cамый холодный 7-дневный промежуток с мая по сентябрь 2021 года: c {date_min} по {date_min+7} {mesyac_min1} месяца")
